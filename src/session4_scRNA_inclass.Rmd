...
dittoDimPlot(object = Tcells, var = "Tage", size = 1.5, reduction.use = "umap", colors = c(1,9,2,10,3,11), main = "Tcells lineage and age")
...
main_umap = dittoDimPlot(Tcells,
                         ...
                         color.panel = dittoColors()[c(cols)], main = NULL) +
...
hightlight_cluster <- function(selected_tage, selected_color){
  dittoDimPlot(Tcells, var = "Tage", size = 1, reduction.use = "umap",
               cells.use = Tcells$Tage == selected_tage, colors = selected_color,
               legend.show = FALSE, ylab = NULL, xlab = NULL, main = NULL, show.axes.numbers = FALSE)
}
...
#apply highlight code to single Tage category
hightlight_cluster(tage[1])

plot_list <- c()

#convert this to a loop
for (x in 1:6) {
  single_plot = hightlight_cluster(selected_tage = tage[x],
